"""Ad-group-scoped entity command helpers."""
...
def register_negatives_adgroup_commands(adgroup_group, ensure_auth_client):
    """Register negative keyword commands scoped to an ad group ID context."""
    ...
    def list_negatives(ctx, show_ids):
        """List negative keywords for this ad group."""
        ...
        if asin_count:
            click.echo(
                ...
                "Use 'amz-ads adgroups <id> negative-asin-targets' to manage competitor ASIN exclusions.",
                err=True,
            )
    ...
    def add_negative(ctx, campaign_id, keyword_text, match_type):
        """Add a negative keyword to this ad group."""
        _, client = ensure_auth_client(ctx)
        ad_group_id = ctx.obj["ad_group_id"]
...
def register_asin_targets_adgroup_commands(adgroup_group, ensure_auth_client):
    """Register ASIN target commands scoped to an ad group ID context."""

    @adgroup_group.group("asin-targets")
    def asin_targets():
    ...
    def list_asin_targets(ctx):
        """List ASIN targets for this ad group."""
        _, client = ensure_auth_client(ctx)
        ad_group_id = ctx.obj["ad_group_id"]
        ...
        click.echo(f"\n{'Target ID':<20} {'ASIN':<15} {'Bid':<8} {'State'}")
        click.echo("-" * 60)
        for target in asin_targets_list:
            ...
            click.echo(f"{target_id:<20} {asin:<15} {bid:<8} {state}")
    ...
    @click.pass_context
    @handle_errors
    def add_asin_target(ctx, campaign_id, asin, bid):
        """Add an ASIN target to this ad group."""
        _, client = ensure_auth_client(ctx)
        ad_group_id = ctx.obj["ad_group_id"]
...
def register_negative_asin_targets_adgroup_commands(adgroup_group, ensure_auth_client):
    """Register negative ASIN target commands scoped to an ad group ID context."""

    @adgroup_group.group("negative-asin-targets")
    ...
    def list_negative_asin_targets(ctx):
        """List negative ASIN targets for this ad group."""
        _, client = ensure_auth_client(ctx)
        ad_group_id = ctx.obj["ad_group_id"]
        ...
        click.echo(f"\n{'Target ID':<20} {'ASIN':<15} {'Type':<20} {'State'}")
        click.echo("-" * 75)
        for target in targets:
            ...
            state = target.get("state", "N/A")
            click.echo(f"{target_id:<20} {asin:<15} {expr_type:<20} {state}")
    ...
    @click.pass_context
    @handle_errors
    def add_negative_asin_target(ctx, campaign_id, asin):
        """Add a negative ASIN target to this ad group."""
        _, client = ensure_auth_client(ctx)
        ad_group_id = ctx.obj["ad_group_id"]
...
def register_keywords_adgroup_commands(adgroup_group, ensure_auth_client):
    """Register keyword commands scoped to an ad group ID context."""
    ...
    def list_keywords(ctx):
        """List keywords for this ad group."""
        _, client = ensure_auth_client(ctx)
        ad_group_id = ctx.obj["ad_group_id"]
        result = client.list_keywords(body={})
        keywords = [k for k in result.payload.get("keywords", []) if k.get("adGroupId") == ad_group_id]
    ...
    def list_all_keywords(ctx):
        ...
        click.echo(f"\n{'Campaign ID':<20} {'Ad Group ID':<20} {'Keyword':<35} {'Match':<10} {'Bid':<8} {'State'}")
        click.echo("-" * 110)
        for kw in keywords:
            camp_id = kw.get("campaignId", "N/A")[:18]
            ag_id = kw.get("adGroupId", "N/A")[:18]
            text = kw["keywordText"][:33]
            match = kw["matchType"]
            bid = f"${kw['bid']}"
            state = kw["state"]
            click.echo(f"{camp_id:<20} {ag_id:<20} {text:<35} {match:<10} {bid:<8} {state}")
    ...
    @click.pass_context
    @handle_errors
    def add_keyword(ctx, campaign_id, keyword_text, match_type, bid):
        """Add a keyword to this ad group."""
        _, client = ensure_auth_client(ctx)
        ad_group_id = ctx.obj["ad_group_id"]
...
def register_targets_adgroup_commands(adgroup_group, ensure_auth_client):
    """Register product target commands scoped to an ad group ID context."""
    ...
    def list_targets(ctx):
        """List product targets for this ad group."""
        _, client = ensure_auth_client(ctx)
        ad_group_id = ctx.obj["ad_group_id"]
        result = client.list_product_targets(
            body={
        ...
        click.echo(f"\n{'Target ID':<20} {'Expression':<40} {'State'}")
        click.echo("-" * 75)
        for t in targets_list:
            target_id = t.get("targetId", "N/A")[:18]
            expr = str(t.get("expression", []))[:38]
            state = t.get("state", "N/A")
            click.echo(f"{target_id:<20} {expr:<40} {state}")
